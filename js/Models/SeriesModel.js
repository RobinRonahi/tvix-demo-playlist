...
var SeriesModel={
    ...
    getAllMovies:function() {
        console.log('📺 SeriesModel: Getting all series for "All" category...');
        
        // Tüm kategorilerdeki dizileri topla
        var allMovies = [];
        var categories = this.categories.filter(function(category) {
            return category.category_id !== 'favourite' && category.category_id !== 'recent';
        });
        
        console.log('📂 Processing', categories.length, 'categories for all series');
        
        for (var i=0; i < categories.length; i++) {
            if (categories[i].movies && categories[i].movies.length > 0) {
                console.log('📁 Category "' + categories[i].category_name + '" has', categories[i].movies.length, 'series');
                allMovies = allMovies.concat(categories[i].movies);
            }
        }
        
        console.log('📺 Total series found:', allMovies.length);
        
        // Dizi tekrarlarını önlemek için dizi ID'lerine göre tekil hale getir
        var uniqueMovies = [];
        var uniqueIds = {};
        ...
        uniqueMovies.sort(function(a, b) {
            ...
            var aId = parseInt(a.series_id || a.id || 0);
            var bId = parseInt(b.series_id || b.id || 0);
            return bId - aId;
        });
    ...
    insertMoviesToCategories:function(){
        ...
        for(var  i=0;i<categories.length;i++){ // except favourite, and recent movies
            ...
            // Her kategorideki dizileri timestamp'e göre sırala (yeni eklenenler üstte)
            if(categories[i].movies && categories[i].movies.length > 0) {
                console.log('📺 Sorting category "' + categories[i].category_name + '" with', categories[i].movies.length, 'series');
    ...
    // Her kategorideki dizileri timestamp'e göre sıralama fonksiyonu
